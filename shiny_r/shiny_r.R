...
ui <- fluidPage(
    titlePanel("CDC Healthy Places in Alabama by County"),
    sidebarLayout(
        sidebarPanel(
...
server <- function(input, output, session) {
    ...
    df_obesity <- reactive({
        data <- df()
        filter(data, MeasureId == "OBESITY", Data_Value_Type == "Crude prevalence")
    })
    ...
    output$barPlot <- renderPlot({
        ...
        ggplot() +
            ...
            labs(title = 'CDC Healthy Places in Alabama by County',
